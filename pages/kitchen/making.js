...
const Making = ({}) => {
  ...
  return (
    <div className="flex h-full w-full flex-1 gap-7.5 overflow-x-auto px-12 pt-7.5">
      {orders
        .filter((x) => x.status === 1)
        .map((order) => (
          <div key={order.orderid} className="flex  flex-col rounded-lg bg-gray-800 " style={{ width: '770px' }}>
            <div className="p-8">
              <div>
